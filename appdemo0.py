...
#         r = level * (8 / max_depth)  # Adjusted radius scaling
#         x = r * math.cos(angle)
#         y = r * math.sin(angle)
#         node_x.append(x)
#         node_y.append(y)
#         node_text.append(item['name'])
#         node_colors.append(max_depth - level)  # Reverse color scale
#         if parent:
...
#                 child_end = child_start + angle_step * 0.9  # Reduce overlap
...
# # Modify the fetch_codevul_xai function
...
#         plotly_chart = generate_chart(output_json)
        
#         return plotly_chart, plot_image
#     else:
#         return "Error fetching results", None
    
# Update the path to your MSR JSON file

MSR_JSON_PATH = "xaiserver/e2e-process/flower_json/msr.json"
MSR_SIMILARITY_PLOT_PATH = "xaiserver/e2e-process/final_diagram/similarity_matrix.png"
...
def load_and_plot_msr_data():
    if not os.path.exists(MSR_JSON_PATH):
        return "Error: MSR JSON file not found", None

    with open(MSR_JSON_PATH, 'r') as f:
        msr_data = json.load(f)

    plotly_chart = generate_chart(msr_data)
    
    # Load similarity matrix image
    if os.path.exists(MSR_SIMILARITY_PLOT_PATH):
        similarity_plot = Image.open(MSR_SIMILARITY_PLOT_PATH)
    else:
        similarity_plot = None

    return plotly_chart, similarity_plot
...
def fetch_codevul_xai(dataset):
    ...
    if response.status_code == 200:
        ...
        output_json = result["output_json"]
        plot_image_path = result["plot_image"]
        plot_image = Image.open(BytesIO(requests.get(plot_image_path).content))
...
def process_dataset(dataset_choice, custom_dataset):
    if dataset_choice == "MSR":
        return load_and_plot_msr_data()
    elif dataset_choice == "Custom" and custom_dataset:
        return fetch_codevul_xai(custom_dataset)
    else:
